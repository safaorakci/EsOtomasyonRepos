...
public partial class System_root_ajax_islem1 : System.Web.UI.Page
{
    ...
    public void is_kaydini_duzenle()
    {
        ...
        dyeni_is_baslangic_tarihi.Text = Convert.ToDateTime(iss["baslangic_tarihi"]).ToShortDateString();
        dyeni_is_baslangic_saati.Text = iss["baslangic_saati"].ToString().Trim().Substring(0, 5);
        dyeni_is_bitis_tarihi.Text = Convert.ToDateTime(iss["bitis_tarihi"]).ToShortDateString();
    ...
    public void is_listesi()
    {
        ...
        {
            sql_str = " and isler.durum = 'true' and case when isler.durum = 'false' then 'İPTAL' when ISNULL(isler.tamamlanma_orani,0)= 100 then 'BİTTİ' when getdate() > convert(datetime, isler.bitis_tarihi) + CONVERT(datetime, isler.bitis_saati) then 'GECİKTİ' when ISNULL(isler.tamamlanma_orani,0)= 0 then 'BEKLİYOR' when ISNULL(isler.tamamlanma_orani,0)< 100 then 'DEVAM EDİYOR' end = 'DEVAM EDİYOR'";
        }
        else if (durum == "biten")
        {
        ...
        if (tip != "")
        {
            ...
            {
                ayarlar.cmd.CommandText = "select isnull(isler.renk, '') as renk, Replace(Replace( STUFF(((select '~<span class=\"hiddenspan\">' + replace(replace(etiket.adi, '(', ''), ')','') + '</span>' from etiketler etiket with(nolock) where CHARINDEX(',' + isnull(etiket.sorgu, '') + ',', ',' + isnull(isler.departmanlar, '') + ',')>0 for xml path(''))), 1, 1, ''), '&lt;', '<'), '&gt;', '>') hidden_etiketler,  case when isler.durum = 'false' then 'İPTAL' when ISNULL(isler.tamamlanma_orani,0)= 100 then 'BİTTİ' when getdate() > convert(datetime, isler.bitis_tarihi) + CONVERT(datetime, isler.bitis_saati) then 'GECİKTİ' when ISNULL(isler.tamamlanma_orani,0)= 0 then 'BEKLİYOR' when ISNULL(isler.tamamlanma_orani,0)< 100 then 'DEVAM EDİYOR' end as is_durum, (select CONVERT(nvarchar(50), kullanici.id) + '~' + isnull(kullanici.personel_resim,'') + '~' + isnull(kullanici.personel_ad,'') + ' ' + isnull(kullanici.personel_soyad,'') + '|' from ucgem_firma_kullanici_listesi kullanici with(nolock) where (SELECT COUNT(value) FROM STRING_SPLIT(isler.gorevliler, ',') WHERE value = CONVERT(NVARCHAR(50), kullanici.id) ) > 0 for xml path('')) as gorevli_personeller, STUFF(((select '~' + etiket.adi from etiketler etiket with(nolock) where CHARINDEX(',' + isnull(etiket.sorgu, '') + ',', ',' + isnull(isler.departmanlar, '') + ',')>0 for xml path(''))), 1, 1, '') as departman_isimleri, ekleyen.personel_ad + ' ' + ekleyen.personel_soyad as ekleyen_adsoyad, isler.* from ucgem_is_listesi isler with(nolock) join ucgem_firma_kullanici_listesi ekleyen with(nolock) on ekleyen.id = isler.ekleyen_id where isler.durum = 'true' and isler.firma_id = @firma_id and ((SELECT COUNT(value) FROM STRING_SPLIT(isler.gorevliler, ',') WHERE value = CONVERT(NVARCHAR(50), @kullanici_id) ) > 0) and case when isler.durum = 'false' then 'İPTAL' when ISNULL(isler.tamamlanma_orani,0)= 100 then 'BİTTİ' when getdate() > convert(datetime, isler.bitis_tarihi) + CONVERT(datetime, isler.bitis_saati) then 'GECİKTİ' when ISNULL(isler.tamamlanma_orani,0)= 0 then 'BEKLİYOR' when ISNULL(isler.tamamlanma_orani,0)< 100 then 'DEVAM EDİYOR' end = 'DEVAM EDİYOR' and isler.cop = 'false' order by (convert(datetime, isler.guncelleme_tarihi) + convert(datetime, isler.guncelleme_saati)) desc;";
            ...
            else if (tip == "arama")
            {
                ...
                if (departmanlar != "0")
                {
                    // sql_str2 += " and dbo.iceriyormu(isler.departmanlar, 'departman-" + departmanlar + "') = 1";
                    sql_str2 += " and (SELECT COUNT(value) FROM STRING_SPLIT(isler.departmanlar, ',') WHERE value = 'departman-" + departmanlar + "') > 0";
                }

                if (santiyeler != "0")
                {
                    // sql_str2 += " and dbo.iceriyormu(isler.departmanlar, 'proje-" + santiyeler + "') = 1";
                ...
                if (toplantilar != "")
                {
                    //sql_str2 += " and dbo.iceriyormu(isler.departmanlar, 'toplanti-" + toplantilar + "') = 1";
                    sql_str2 += " and (SELECT COUNT(value) FROM STRING_SPLIT(isler.departmanlar, ',') WHERE value = 'toplanti-" + toplantilar + "') > 0";
                }

                if (gorevliler != "0")
                {
                    // sql_str2 += " and dbo.iceriyormu(isler.gorevliler, '" + gorevliler + "') = 1";
        ...
        foreach (DataRow item in ds.Tables[0].Rows)
        {
            ...
            yeniis.tamamlanma_orani = Convert.ToInt32(item["tamamlanma_orani"].Equals(DBNull.Value) ? 0 : item["tamamlanma_orani"]);
        ...
        SqlDataAdapter sda_tablo = new SqlDataAdapter(ayarlar.cmd);
        DataSet ds_tablo = new DataSet();
        sda_tablo.Fill(ds_tablo);*/
    ...
    [WebMethod(EnableSession = true)]
    public static string IsGuncelle(int IsID, string adi, string aciklama, string gorevliler, string departmanlar, string oncelik, string kontrol_bildirim, string baslangic_tarihi, string baslangic_saati, string bitis_tarihi, string bitis_saati, string renk, string ajanda_gosterim, string gunluk_sure, string toplam_sure, string toplam_gun, int sinirlama_varmi)
    {
        string durum = "true";
        try
        {
                ...
                if (gorevliID.Length > 0)
                {
                    if (ayarlar.IsNumeric(gorevliID))
                    {
                        if (Convert.ToInt32(gorevliID) > 0)
                        {
                            ...
                            int varmi = Convert.ToInt32(ayarlar.cmd.ExecuteNonQuery());
                            ...
                            else
                            {
                                ayarlar.baglan();
                                ayarlar.cmd.Parameters.Clear();
                                ayarlar.cmd.CommandText = "update ucgem_is_gorevli_durumlari set gunluk_sure = @gunluk_sure, toplam_sure = @toplam_sure, toplam_gun = @toplam_gun where id = @ID ";
                                ayarlar.cmd.Parameters.Add("ID", varmi);
                                ayarlar.cmd.Parameters.Add("gunluk_sure", gunluk_sure);
                                ayarlar.cmd.Parameters.Add("toplam_sure", toplam_sure);
                                ayarlar.cmd.Parameters.Add("toplam_gun", toplam_gun);
                                ayarlar.cmd.ExecuteNonQuery();
    ...
    public void yeni_is_ekle()
    {
        ...
        yeni_is_ekle_button.Text = LNG("Yeni İş Ekle");
        ayarlar.cnn.Close();
    }
    ...
    public void gorevler()
    {
        ...
        ayarlar.baglan();
        ayarlar.cmd.Parameters.Clear();
        ayarlar.cmd.CommandText = "select * from tanimlama_gorev_listesi where firma_id = @firma_id and cop = 'false'";
        ayarlar.cmd.Parameters.Add("firma_id", SessionManager.CurrentUser.firma_id);
        SqlDataAdapter sda = new SqlDataAdapter(ayarlar.cmd);
        DataSet ds = new DataSet();
        sda.Fill(ds);
    ...
    public void firmalar()
    {
        ...
        if (islem2 == "ekle")
        {
            ...
            ayarlar.cmd.CommandText = "update ucgem_firma_listesi set firma_hid = @firma_hid where id = @firma_id;";
        ...
        {
            ayarlar.cmd.CommandText = "select kullanici.personel_ad + ' ' + kullanici.personel_soyad supervisor, firma.* from ucgem_firma_listesi firma join ucgem_firma_kullanici_listesi kullanici with(nolock) on kullanici.id = firma.firma_supervisor_id where firma.cop = 'false' and firma.yetki_kodu = @yetki_kodu and firma.firma_hid like '" + SessionManager.CurrentUser.firma_hid + ".%';";
        }
