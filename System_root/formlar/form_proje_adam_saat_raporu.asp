    ...
    <body>
        ...
        <tr>
            <td style="vertical-align: top; text-align: left; line-height: 20px;">
                <h2><%=LNG("PROJE ADAM-SAAT RAPORU")%></h2>
                            ...
                            <table id="tablegosterge" style="border-color: #e8e8e8; font-family: Tahoma; width:100%;">
                                ...
                                <tbody id="tbody">
                                    <%               
                                        ...
                                        SQL="EXEC dbo.ProjeAdamSaatCetveliRapor @proje_id = '"& yeni_is_yuku_proje_id &"', @firma_id = '"& firma_id &"', @baslangic = '"& dongu_baslangic &"', @bitis = '"& dongu_bitis &"', @rapor_personel_id = '"& rapor_personel_id &"', @etiketler = '"& etiketler &"';"
                                    ...
                                    <tr class=" ustunegelince <%=klas %>">
                                        ...
                                        <% end if %>
                                        <td class="alt_td <% if day(cetvel("tarih"))=1 then %> alt_td2 <% end if %> <% if cdate(cetvel("tarih"))=cdate(date) then %> sarialan <% end if %> "><% if cdbl(cetvel("maliyet_tutari"))=0 then %>-<% else %><%=DakikadanSaatYap(cetvel("kaynak_toplam_saat")) %><br /><%=cetvel("maliyet_tutari") %>TL<% end if %></td>
                                        <%  
