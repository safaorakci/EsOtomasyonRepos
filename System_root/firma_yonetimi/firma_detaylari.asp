    ...
    <div class="col-lg-12">
        <div class="tabs tabs-style-bar">
            <nav>
                <ul>
                    <%
                        TaseronMusteri = False
                        TaseronCari = False
                        TaseronZimmet = False
                        TaseronDosyalar = False
                        TaseronAjanda = False
                        TaseronisListesi = False
                        TaseronAdamSaat = False
                        TaseronRaporlar = False

                        MusteriCari = False
                        MusteriDosyalar = False
                        MusteriAjanda = False
                        MusteriisListesi = False
                        MusteriRaporlar = False
                        if not tblModulYetkiler.eof then
                            t = 0
                            m = 0
                            do while not tblModulYetkiler.eof
                                if tblModulYetkiler("ModulId") = 19 and tblModulYetkiler("Status") = True then
                                    TaseronCari = True
                                    t = t + 1
                                end if
                                if tblModulYetkiler("ModulId") = 20 and tblModulYetkiler("Status") = True then
                                    TaseronZimmet = True
                                    t = t + 1
                                end if
                                if tblModulYetkiler("ModulId") = 21 and tblModulYetkiler("Status") = True then
                                    TaseronDosyalar = True
                                    t = t + 1
                                end if
                                if tblModulYetkiler("ModulId") = 22 and tblModulYetkiler("Status") = True then
                                    TaseronAjanda = True
                                    t = t + 1
                                end if
                                if tblModulYetkiler("ModulId") = 23 and tblModulYetkiler("Status") = True then
                                    TaseronisListesi = True
                                    t = t + 1
                                end if
                                if tblModulYetkiler("ModulId") = 24 and tblModulYetkiler("Status") = True then
                                    TaseronAdamSaat = True
                                    t = t + 1
                                end if
                                if tblModulYetkiler("ModulId") = 25 and tblModulYetkiler("Status") = True then
                                    TaseronRaporlar = True
                                    t = t + 1
                                end if

                                if tblModulYetkiler("ModulId") = 11 and tblModulYetkiler("Status") = True then
                                    MusteriCari = True
                                    m = m + 1
                                end if
                                if tblModulYetkiler("ModulId") = 13 and tblModulYetkiler("Status") = True then
                                    MusteriDosyalar = True
                                    m = m + 1
                                end if
                                if tblModulYetkiler("ModulId") = 14 and tblModulYetkiler("Status") = True then
                                    MusteriAjanda = True
                                    m = m + 1
                                end if
                                if tblModulYetkiler("ModulId") = 15 and tblModulYetkiler("Status") = True then
                                    MusteriisListesi = True
                                    m = m + 1
                                end if
                                if tblModulYetkiler("ModulId") = 17 and tblModulYetkiler("Status") = True then
                                    MusteriRaporlar = True
                                    m = m + 1
                                end if
                    ...
                    <%if TaseronZimmet = True then %>
                    <% if instr(Request.Cookies("kullanici")("yetkili_sayfalar"), ",111,")>0 then %>
                    <li class="nav-link_yeni"><a href="#zimmet" onclick="firma_zimmet_getir('firma', '<%=firma_id %>', this);" class="icon icon-display"><span><%=LNG("Zimmet")%></span></a></li>
                    <% end if %>
                    <%end if %>

                    <%if TaseronDosyalar = True then %>
                    ...
                    <%if TaseronAdamSaat = True then %>
                    <% if instr(Request.Cookies("kullanici")("yetkili_sayfalar"), ",112,")>0 then %>
                    <li class="nav-link_yeni"><a href="#adamsaat" onclick="taseron_adamsaat_getir('<%=firma_id %>', this, '<%=month(date) %>', '<%=year(date) %>');" class="icon icon-box"><span><%=LNG("Adam Saat")%></span></a></li>
                    <% end if %>
                    <%end if %>

                    <%if TaseronRaporlar = True then %>
                    ...
                    <li class="nav-link_yeni" <%if m < 1 then%> style="display:none" <%end if %>><a style="-webkit-border-top-left-radius: 10px; -webkit-border-bottom-left-radius: 10px; -moz-border-radius-topleft: 10px; -moz-border-radius-bottomleft: 10px; border-top-left-radius: 10px; border-bottom-left-radius: 10px;" href="#musteri_bilgileri" onclick="musteri_bilgilerini_getir('<%=firma_id %>', this);" class="icon icon-home"><span><%=LNG("Müşteri")%></span></a></li>

                    <%if MusteriCari = True then %>
                    <% if instr(Request.Cookies("kullanici")("yetkili_sayfalar"), ",98,")>0 then %>
                    <li class="nav-link_yeni"><a href="#cari_hareketler" onclick="musteri_cari_hareketleri_getir('<%=firma_id %>', this);" class="icon icon-box"><span><%=LNG("Cari Hareketler")%></span></a></li>
                    <% end if %>
                    <%end if %> 

                    <%if MusteriDosyalar = True then %>
                    <% if instr(Request.Cookies("kullanici")("yetkili_sayfalar"), ",99,")>0 then %>
                    <li class="nav-link_yeni"><a href="#dosyalar" onclick="musteri_dosyalari_getir('<%=firma_id %>', this);" class="icon icon-upload"><span><%=LNG("Dosyalar")%></span></a></li>
                    <% end if %>
                    <%end if %>

                    <%if MusteriAjanda = True then %>
                    <% if instr(Request.Cookies("kullanici")("yetkili_sayfalar"), ",100,")>0 then %>
                    <li class="nav-link_yeni"><a href="#ajanda" onclick="musteri_ajanda_getir('<%=firma_id %>', this);" class="icon icon-tools"><span><%=LNG("Ajanda")%></span></a></li>
                    <% end if %>
                    <%end if %>

                    <%if MusteriisListesi = True then %>
                    <% if instr(Request.Cookies("kullanici")("yetkili_sayfalar"), ",101,")>0 then %>
                    <li class="nav-link_yeni"><a href="#is_listesi_panel" onclick="musteri_is_listesi_getir('<%=firma_id %>', this);" class="icon icon-home"><span><%=LNG("İş Listesi")%></span></a></li>
                    <% end if %>
                    <%end if %>

                    <%if MusteriRaporlar = True then %>
                    <% if instr(Request.Cookies("kullanici")("yetkili_sayfalar"), ",102,")>0 then %>
                    <li class="nav-link_yeni"><a onclick="musteri_raporlarini_getir('<%=firma_id %>', this, this);" style="-webkit-border-top-right-radius: 10px; -webkit-border-bottom-right-radius: 10px; -moz-border-radius-topright: 10px; -moz-border-radius-bottomright: 10px; border-top-right-radius: 10px; border-bottom-right-radius: 10px;" href="#raporlar" class="icon icon-tools"><span><%=LNG("Raporlar")%></span></a></li>
                    <% end if %>
                    <%end if %>
