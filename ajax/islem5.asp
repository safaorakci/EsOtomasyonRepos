...
<form autocomplete="off" id="talepfisform" class="smart-form validateform" novalidate="novalidate" style="padding: 15px;">
    ...
    <div class="row">
        ...
        <div class="col-sm-12">
            <input type="file" id="talep_dosya" yol="/Parcalar" tip="kucuk" class="form-control" />
        </div>
    </div>
...
<form autocomplete="off" id="talepfisform" class="smart-form validateform" novalidate="novalidate" style="padding: 15px;">
    ...
    <div class="row">
        <label class="col-sm-12 col-form-label">Dosya :</label>
        <div class="col-sm-12">
            <input type="file" id="talep_dosya" yol="/Parcalar" tip="kucuk" value="<%=talepcek("dosya") %>" class="form-control" />
        </div>
    </div>
...
<%
    elseif trn(request("islem"))="is_timer_start_kaydi" then
        ...
        if trn(request("islem2"))="baslat" then
            ...
            SQL ="SELECT COUNT(*) FROM ucgem_is_calisma_listesi WHERE baslangic IS NOT NULL AND bitis IS NULL AND ekleyen_id = '"& ekleyen_id &"'"
...
<table class="table">
    ...
    <tbody>
        ...
        %>
        <tr>
            <td><%=cdate(left(calisma("baslangic"),10)) %>&nbsp;<%=hour(calisma("baslangic")) %>:<%=minute(calisma("baslangic")) %></td>
            <td>
                <% if isnull(calisma("bitis"))=false then  %>
                <%=cdate(left(calisma("bitis"),10)) %>&nbsp;<%=hour(calisma("bitis")) %>:<%=minute(calisma("bitis")) %>
                <% else 
                    girdimi = true

                        fromDate=cdate(left(calisma("baslangic"),10)) & " " & hour(calisma("baslangic")) & ":" & minute(calisma("baslangic")) & ":" & second(calisma("baslangic"))
                        toDate=now()

                    baslangic_tarihi = cdate(left(calisma("baslangic"),10))
                    baslangic_saati = hour(calisma("baslangic")) & ":" & minute(calisma("baslangic"))
                %>
                    ---
        ...
        <%
            ...
            if cdbl(toplamdakika)>0 then

                toplamsaat2 = ((toplamdakika - toplamdakika mod 60) / 60)
                toplamdakika2 = toplamdakika mod 60

                if cdbl(toplamsaat2)<10 then
                    toplamsaat2 = "0" & toplamsaat2
                end if

                if cdbl(toplamdakika2)<10 then
                    toplamdakika2 = "0" & toplamdakika2
                end if
        ...
        <tr>
            <td colspan="2"></td>
            <td><b><%=toplamsaat2 %>:<%=toplamdakika2 %>:00</b></td>
        </tr>
    </tbody>
</table>
<% if girdimi = true then 
    ...
    <tbody>
        ...
        <tr>
            ...
            <td style="width: 10px;">
                ...
                <input type="button" class="btn btn-primary btn-mini" value="<%=LNG("İş Emri Oluştur")%>" onclick="BakimdanIsEmriOlustur('<%=bakim("proje_id") %>', '<%=bakim("id") %>', '<%=Tum %>');" />
