...
<html lang="tr">
<head>
    <title><%=LNG("Departman Adam-Saat Raporu")%></title>
    ...
    <table style="width: 100%;">
        <tr>
            <td style="vertical-align: top; text-align: left;">
                <h2><%=LNG("DEPARTMAN ADAM-SAAT RAPORU")%></h2>
                ...
                <div id="tablediv">
                    <table id="tablegosterge" style="border-color: #e8e8e8; font-family: Tahoma; width: 100%;">
                        ...
                        <tbody id="tbody">
                            ...
                            <%
                                        SQL="EXEC dbo.DepartmanAdamSaatCetveli @proje_id = '"& proje_id &"', @firma_id = '"& firma_id &"', @baslangic = '"& cdate(dongu_baslangic) &"', @bitis = '"& cdate(dongu_bitis) &"', @etiketler = '"& etiketler &"', @rapor_personel_id = '"& rapor_personel_id &"';"
                            ...
                            <tr class=" ustunegelince <%=klas %>">
                                ...
                                <td class=" gosterge_td alt_td sagcizgi"><%=cetvel("kaynak_toplam_maliyet") %> ₺</td>
                                <% end if %>
                                <td class="alt_td <% if day(cetvel("tarih"))=1 then %> alt_td2 <% end if %> <% if cdate(cetvel("tarih"))=cdate(date) then %> sarialan <% end if %> "><% if cdbl(cetvel("maliyet_tutari"))=0 then %>-<% else %><%=cetvel("maliyet_tutari") %>₺<% end if %></td>
                                <%  
                            ...
                            <tr>
                                ...
                                <td class=" gosterge_td alt_td sagcizgi"><%=toplam_tutar %> ₺</td>
                                <% for x = 0 to ubound(gun_toplam)-1 %>
                                <td class="alt_td" style="background-color: #4d7193; color: white;"><%=gun_toplam(x) %>₺</td>
                                <% next %>
                            </tr>
                            <%
                                Erase gun_toplam
